export default function Home() {
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-r from-blue-500 to-purple-600 text-white p-6">
      <h1 className="font-bold text-5xl mb-12 text-center ">
        Supercharge Your Apps with Clerk + Next.js!
      </h1>
      <p className="text-lg max-w-2xl text-justify mb-6  ">
        This template is built using Next.js and Clerk for seamless
        authentication. It&apos;s designed to help you quickly set up your own
        applications with ease.
      </p>
      <p className="text-lg max-w-2xl text-justify mb-8">
        Whether you&apos;re building a personal project or a community-driven
        platform, our template provides a solid foundation to get started.
      </p>
    </div>
  );
}
